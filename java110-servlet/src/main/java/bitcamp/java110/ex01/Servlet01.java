...
public class Servlet01 implements Servlet {
    ...
    public Servlet01() {
        System.out.println("Servlet01() 호출 됨");
    }
    
    @Override
    public void init(ServletConfig config) throws ServletException {
        // 서블릿 객체를 만든 후 생성자 다음으로 호출되기 때무ㅐㄴ에
        // 이 메서드에서는 서블릿이 작업할 때 사용할 자원을 준비하는 일을 한다.
        // 예를 들면, DB 커넥션을 준비한다거나 외부 서버에 소켓을 연결하는 등의 작업이다.
        // 또는 서블릿에서 사용할 서비스 객체나 DAO 객체를 준비하는 일을 ㅇ한다
    ...
    public ServletConfig getServletConfig() {
        ...
        // 그래서 이 메서드는 init()에서 파라미터로 받은 ServletConfig를 객체로
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 에를 들면, 게시물 목록을 출력하거나 로그인을 처리하거나 회원가입을 처리하는 등의 일을 한다. 
        System.out.println("service() 호출됨.");
    }

    @Override
    public String getServletInfo() {
        // 서블릿을 실행 중에 직업 호출할 일은 없다.
    ...
    public void destroy() {
        ...
        // 그래서 이 메서드에서는 init()에서 준비한 자원을 해체시키는 일은 을 한다.
        // 예를 들면 DB커넥션을 닫는다거나 소켓 연결을 닫는등의 작업을 수행한다.
...
//      - 생성된 몯느 서블릿들에 대해 destroy() 호출
//      - 생성된 모든 서블릿 객체를 가비지로 만든다.
// 주의!
// - 서블릿 객체는 클래스 당 한개만 생성된다.
// - 요청할 때마다 생성되는 것이 아니다.
