...
public class Filter01 implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // 서블릿을 실행하기 전에 하는일 :
        // - 클라이언트가 암호화해서 보낸 데이터를 서블릿이 사용할 수 있도록 암호 해체하기
        // - 클라이언트가 압축해서 보낸 데이털르 서블릿이 사용할 수 있도록 압축해체 하기
        // - 클라이언트의 요청에 대해 기록을 남기기
        // - 클라이언트가 특별한 형식으로 보낼 데이터를 파싱하여 서블릿이 사용하기 쉽게 변환하기
        // - 클라이언트가 요청한 서블릿의 실행 권환이 있는지 검사한기
        // - 클라이언트가 로그인 한 사용자인지 검사하기
        System.out.println("Filter01.doFilter() --- before");
        
        // => 다음 필터가 있다면 실행하고 없다면 서블릿을 호출하게 한다.
        chain.doFilter(request, response);
        // chain을 주석으로 처리하면 reloading이  뜬다. 서비스를 호출하는 역할, 없으면 안된다.
        // 위 메서드를 호출하지 않으면 다음 필터나 서블릿을 시행하지 않는다.
        // 따라서 빼면 안된다.
