...
- 문법
    &lt;jsp:useBean 
     ...
     - scope를 지정하지 않으면 다음 순서로 객체를 찾는다. 지정하지 않으면 기본값은 page이다.
       page --> request --> session --> application
  class:
     - 보관된 객체의 타입을 가리킨다.
     - fully-qualified class name 이다. 
        - 즉 패키지명을 포함한 클래스 이름 이어야한다.
        - page의 import 속성에 패키지 이름이 지정된 것과 상관없이
        - 반드시 패키지명을 지정해야 한다.
     - 보관소에서 꺼낸 객체를 저장하기 위한 변수의 타입으로 사용된다.
     - 만약 보관소에 지정된 이름의 객체가 없다면, 이속성에 지정된 객체를 임의로 만들것이다.
</pre>
<%
// ServletContext 보관소
...
%>
<jsp:useBean scope="session" id="name" class="java.lang.String"/>
