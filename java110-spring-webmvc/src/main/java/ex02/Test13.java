...
public class Test13 {
    ...
    // 라는 오류는 변수 타입에 맞지않게 입력을 했다는 내용이다. 
    @RequestMapping("m2")
    public String m2(
    ...
    // 값을 생략했을 때 기본 값을 갖게 하고 싶다면 defaultValue 속성을ㅇ 지정하라.
    @RequestMapping("m3")
    public String m3(
            @RequestParam(name="name", required=false) String name,
    ...
    // @RequestMapping에서 이름을 지정할 필요가 없다.
    @RequestMapping("m4")
    public String m4(
    ...
    // @RequestMapping을 생략하면 다음을 선언한 것과 같다
    // @RequestMapping(value="변수명", required=false) 
    @RequestMapping("m5")
    public String m5(
            String name,
            @RequestParam(defaultValue="20") int age, // age 은 기본값을 주어서 requestParam을 안뺏음
            String tel,
            Model model
            ) {
        model.addAttribute("name",name);
