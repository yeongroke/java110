...
public class Test07 {
    ...
    // HTTP 클라이언트 서버에 데이터를 보낼 때 그 형식을 MINE 타입으로 표현해서 전달한다.
    // => Content-Type 헤더를 사용하여 설정한다
    // 테스트:
    // http://localhost:8888/ex02/Test07.html
    @RequestMapping(value="/ex02/test07",consumes="text/plain")
    @ResponseBody
    ...
    @RequestMapping(value="/ex02/test07",consumes="text/csv")
    @ResponseBody
    public String m2(String content) {
        System.out.println(content);
        return "ex02.Test07.m2";
    }
    
    @RequestMapping(value="/ex02/test07",consumes="application/x-www-form-urlencoded")
    @ResponseBody
    ...
    @RequestMapping(value="/ex02/test07",consumes="application/json")
    @ResponseBody
